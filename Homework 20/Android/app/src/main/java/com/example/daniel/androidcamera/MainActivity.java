package com.example.daniel.androidcamera;
// libraries
...
//        myScrollView = (ScrollView) findViewById(R.id.ScrollView01);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                myTextView2.setText("value of PWM is "+PWM);
...
//        parameters.setAutoExposureLock(true); // keep the white balance constant
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        ...
        if (c != null) {
            ...
            for (int startY = 100; startY <= 200; startY+=10) {
...
//                        pixels[i] = rgb(255, 0, 0); // over write the pixel with pure red
                ...
                if (counter != 0) {
                    COM[(startY/10)-10] = middle / counter;
//                    canvas.drawCircle(COM[(startY/10)-10], startY,5 , paint1); // x position, y position, diameter, color
                }

                // update the row
//                bmp.setPixels(pixels, 0, bmp.getWidth(), 0, startY, bmp.getWidth(), 1);
            }
//            if (abs(COM[0]-COM[10])< 300) {
                PWM = (COM[0] + COM[1] + COM[2] + COM[3] + COM[4] + COM[5] + COM[6] + COM[7] + COM[8] + COM[9] + COM[10]) / 11;
//            }
            String sendString = String.valueOf(PWM) + '\n';
            try {
                sPort.write(sendString.getBytes(), 10); // 10 is the timeout
...
//        canvas.drawText("pos = 0", 10, 200, paint1);
